...
cl = LINETCR.LINE()
#cl.login(qr=True)
cl.login(token="Er24rbqaLS5F63fnKrg1.8nezDhK7HGW9hVtLK4qvGq.saYwMpi2LyHrh0cexBMFEmAMwPggIxTdWNN9pUEc4oI=")
cl.loginResult()

ki = LINETCR.LINE()
#ki.login(qr=True)
ki.login(token="ErrIdg2ghP8D9XRVshKd.5ws3BZPRgZdFiDAuWEqM+q.mJyLm98r7ZjVeK8hOVx9AuZBUodU8+PhdK/zERpDs3c=")
ki.loginResult()

kk = LINETCR.LINE()
#kk.login(qr=True)
kk.login(token="ErDqcmO1nVlHDmDFzSee.AKJjgcQRejNwIdH96qxmZG.DQFqKziSxM5AHgRuzR8RvjH1FZjz0D95uuS0wakdcMc=")
kk.loginResult()

kc = LINETCR.LINE()
#kc.login(qr=True)
kc.login(token="ErrIdg2ghP8D9XRVshKd.5ws3BZPRgZdFiDAuWEqM+q.mJyLm98r7ZjVeK8hOVx9AuZBUodU8+PhdK/zERpDs3c=")
kc.loginResult()

ks = LINETCR.LINE()
#ks.login(qr=True)
ks.login(token="ErmJKlaSRSv54LHjpSr7.B1SXxj/vzd0tDPyYfa16jW.D8PJxIWlQOS6Dq1aHsMcGHlLprsgXTN+kcrEsVAGr/I=")
ks.loginResult()

k1 = LINETCR.LINE()
#k1.login(qr=True)
k1.login(token="ErmJKlaSRSv54LHjpSr7.B1SXxj/vzd0tDPyYfa16jW.D8PJxIWlQOS6Dq1aHsMcGHlLprsgXTN+kcrEsVAGr/I=")
k1.loginResult()

k2 = LINETCR.LINE()
#k2.login(qr=True)
k2.login(token="ErmJKlaSRSv54LHjpSr7.B1SXxj/vzd0tDPyYfa16jW.D8PJxIWlQOS6Dq1aHsMcGHlLprsgXTN+kcrEsVAGr/I=")
k2.loginResult()

k3 = LINETCR.LINE()
#k3.login(qr=True)
k3.login(token="ErmJKlaSRSv54LHjpSr7.B1SXxj/vzd0tDPyYfa16jW.D8PJxIWlQOS6Dq1aHsMcGHlLprsgXTN+kcrEsVAGr/I=")
k3.loginResult()

satpam = LINETCR.LINE()
satpam.login(token="ErmJKlaSRSv54LHjpSr7.B1SXxj/vzd0tDPyYfa16jW.D8PJxIWlQOS6Dq1aHsMcGHlLprsgXTN+kcrEsVAGr/I=")
...
KIC=[cl,ki,kk,kc,ks]
KAC=[cl,ki,kk,kc,ks,k1,k2,k3,satpam]
DEF=[kc,ks]
                   ...
                   if op.param1 in cctv['point']:
                       ...
                       if Name in cctv['sidermem'][op.param1]:
                           ...
                           if " " in Name:
                               ...
                               nick = Name.split(' ')
                               if len(nick) == 2:
                                   random.choice(KIC).sendText(op.param1, "Haii kak" + "☞ " + nick[0] + " ☜" + "\nNgintip Aja Niih. . .\nChat Kek Idiih (-__-)  \nJam :" + datetime.today().strftime('%H:%M:%S'))
                               else:
                                   random.choice(KIC).sendText(op.param1, "Haii kak" + "☞ " + nick[0] + " + ☜" + "\nSini bareng ngobrol (-_-) \nJam :" + datetime.today().strftime('%H:%M:%S'))
                           else:
                               random.choice(KIC).sendText(op.param1, "Haii kak" + "☞ " + nick[0] + " + ☜" + "\nSini barengngobrol (-_-) \nJam :" + datetime.today().strftime('%H:%M:%S'))
                   else:
                       pass
               else:
                   pass
        ...
        if op.type == 11:
            if op.param3 == '1':
                if op.param1 in wait['pname']:
                    try:
                       G = random.choice(KIC).getGroup(op.param1)
                    except:
                        pass
                    G.name = wait['pro_name'][op.param1]
                    try:
                       random.choice(KIC).updateGroup(G)
                    except:
                        pass
                    if op.param2 in ken:
                        pass
                        ...
                        try:
                           random.choice(KIC).kickoutFromGroup(op.param1,[op.param2])
                            ...
                            pass
                        random.choice(KIC).sendText(op.param1,"Kak jgn ubah nama Group\nMaaf aku kick kamu kak (-_-) \nJam :" + datetime.today().strftime('%H:%M:%S'))
                        c = Message(to=op.param1,from_=None, text=None, contentType=13)
                        c.contentMetadata={'mid':op.param2}
                        random.choice(KIC).sendMessage(c)
...
#--------------------------------
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
               if wait["detectMention"] == True:
                  if wait["kickMention"] == True:
                     ...
                     balas = ["Jangan ngtag hoii!!!" + datetime.today().strftime('%H:%M:%S'), cName + "Ngapain ngetag kak? (-_-) " + datetime.today().strftime('%H:%M:%S'), "Ada apa kak ngetag netag???" + datetime.today().strftime('%H:%M:%S'), "Pm aja kak!\nKalo kangen" + datetime.today().strftime('%H:%M:%S'), "Dia lagi tidur" + datetime.today().strftime('%H:%M:%S'), "Dia lagi offline kak." + datetime.today().strftime('%H:%M:%S'), cName + "Kenapa ngetag kak?" + datetime.today().strftime('%H:%M:%S'), "Ada perlu apa?" + datetime.today().strftime('%H:%M:%S'), "Dia lagi tidur kak" + datetime.today().strftime('%H:%M:%S'), cName + "Jangan ngetag kak, Ribut!!" + datetime.today().strftime('%H:%M:%S'), "Hadi kak, Kangen ya??" + datetime.today().strftime('%H:%M:%S'), "Telpon aja dia kak" + datetime.today().strftime('%H:%M:%S'), cName + "Ngtag mulu lu kak" + datetime.today().strftime('%H:%M:%S'), "Nanti ku kick kamu kak" + datetime.today().strftime('%H:%M:%S'), "Kakak siapa???" + datetime.today().strftime('%H:%M:%S')]
        ...
        if op.type == 26:
            ...
            if msg.contentType == 13:
              ...
              if msg.from_ in admin:
            ...
            elif msg.text in ["Help bot","Key bot"]:
                cl.sendText(msg.to,botMessage)
            elif msg.text in ["Help admin","Key admin"]:
                cl.sendText(msg.to,adminMessage)
            elif msg.text in ["Help owner","Key owner"]:
                cl.sendText(msg.to,ownerMessage)
            elif msg.text in ["Help room","Key room"]:
                ...
                wait["Sider"] = True
                cl.sendText(msg.to,"Siap sider")
#datetime.today().strftime('%H:%M:%S'))
            elif msg.text in ["Sider off"]:
                if msg.to in cctv['point']:
                    cctv['cyduk'][msg.to]=False
                    wait["Sider"] = False
                    cl.sendText(msg.to, "Sider Off")
                else:
                    cl.sendText(msg.to, "Belum di on kan kakak")
...
#random.choice(KAC).sendMessage(msg)
              ...
              if msg.from_ in admin:
                if msg.toType == 2:
                    X = random.choice(KIC).getGroup(msg.to)
                    X.preventJoinByTicket = False
                    random.choice(KIC).updateGroup(X)
                    if wait["lang"] == "JP":
                        random.choice(KIC).sendText(msg.to,"QR Sudah Dibuka")
                    else:
                        random.choice(KIC).sendText(msg.to,"Sudah Terbuka Plak")
                else:
                    if wait["lang"] == "JP":
                        random.choice(KIC).sendText(msg.to,"Can not be used outside the group")
                    else:
                        random.choice(KIC).sendText(msg.to,"Not for use less than group")
              else:
                cl.sendText(msg.to,"Perintah Ditolak.")
              ...
              if msg.from_ in admin:
                if msg.toType == 2:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        random.choice(KIC).sendText(msg.to,"Sudah Tertutup Plak")
                else:
                    if wait["lang"] == "JP":
                        random.choice(KIC).sendText(msg.to,"Can not be used outside the group")
                    else:
                ...
                if msg.from_ in admin:
                  ...
                  if wait["lang"] == "JP":
                    ...
                    if ginfo.preventJoinByTicket == True:
                      ...
                      QR = "Open"
                    random.choice(KIC).sendText(msg.to,"[Group Name]\n" + "[•]" + str(ginfo.name) + "\n\n[Group ID]\n" + msg.to + "\n\n[Group Creator]\n" + "[•]" + gCreator + "\n\n[Group Status]\n" + "[•]Status QR =>" + QR + "\n\n[Group Picture]\nhttp://dl.profile.line.naver.jp/" + ginfo.pictureStatus + "\n\nMembers:" + str(len(ginfo.members)) + "\nPending:" + sinvitee)
                  else:
                    random.choice(KIC).sendText(msg.to,"[Group Name]\n" + str(ginfo.name) + "\n\n[Group ID]\n" + msg.to + "\n\n[Group Creator]\n" + gCreator + "\n\n[Group Status]\nGroup Picture:\nhttp://dl.profile.line.naver.jp/" + ginfo.pictureStatus)
                else:
                  if wait["lang"] == "JP":
...
#time.sleep(0.01)
                ...
                G.preventJoinByTicket = True
                ks.updateGroup(G)
                cl.sendText(msg.to,"Semua Sudah Lengkap")
